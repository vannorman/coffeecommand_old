...
//	public Transform target;
	...
	public Vector3 GetMaxExtendedPosition {
		get {
			return outT.position + (outT.position - transform.position).normalized * .2f; // just PAST the target position to allow for a "pop" effect
		}
	}
	public Vector3 GetMaxExtendedTarget {
		get {
			return outT.position + (outT.position - transform.position).normalized * .4f; // just PAST the target position to allow for a "pop" effect
		}
	}
